# UI Table Developer Guide

TanStack Table wrapper APIs.
...
- You need **table UI**, **data-heavy views**, **shared interactions** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ui-table` instead of reaching into app-specific internals.
...
| `framework/libraries/ui-table` | Nested publishable library package. |
| `framework/libraries/ui-table/src` | Public runtime source and exported modules. |
| `framework/libraries/ui-table/tests` | Unit and contract verification where present. |
...
| Field | Value |
| --- | --- |
| Direct Dependencies | `@tanstack/react-table` |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `column`, `createPlatformTableOptions`, `usePlatformTable`, `createPlatformTableState`, `togglePlatformRowSelection` |
...
- Hosts should import from `@platform/ui-table`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ui-table"]
...
```ts
import { packageId, packageDisplayName, column } from "@platform/ui-table";
...
- Publishes 0 public modules from `@platform/ui-table`.
- Exports 18 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `column`, `createPlatformTableOptions`, `usePlatformTable`, and more.
